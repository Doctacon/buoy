...
from buoy_search.planning.applied_state import applied_state_paths, load_applied_state
...
from buoy_search.relations.base import DatabaseRelationError
...
from buoy_search.planning.plan_cleanup import cleanup_superseded_plan_directories
from buoy_search.planning.plan_diff import IncrementalPlanDiff
...
def source_from_request(request: PlanningRequest) -> object:
    ...
    if backend == "duckdb":
        ...
        from buoy_search.relations.duckdb import duckdb_relation_source
    ...
    if backend == "bigquery":
        from buoy_search.relations.bigquery import bigquery_relation_source
    ...
    from buoy_search.relations.snowflake import snowflake_relation_source
...
def crawl_source_with_plan(source: object, options: CrawlOptions) -> CrawlExecution:
    ...
    if source_kind == "duckdb_relation":
        from buoy_search.relations.duckdb import crawl_duckdb_relation_with_plan

        return crawl_duckdb_relation_with_plan(source, options)  # type: ignore[arg-type,return-value]
    if source_kind == "bigquery_relation":
        from buoy_search.relations.bigquery import crawl_bigquery_relation_with_plan
    ...
    if source_kind == "snowflake_relation":
        from buoy_search.relations.snowflake import crawl_snowflake_relation_with_plan

        return crawl_snowflake_relation_with_plan(source, options)  # type: ignore[arg-type,return-value]
    if isinstance(source, GitHubRepoSource):
        from buoy_search.indexing.github_repo import crawl_github_repo_with_plan

        return crawl_github_repo_with_plan(source, options)
    if isinstance(source, (PdfSource, LocalFileSource)):
        from buoy_search.indexing.crawler import crawl_local_document_with_plan

        return crawl_local_document_with_plan(source, options)
    from buoy_search.indexing.crawler import crawl_site_with_plan
