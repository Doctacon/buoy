...
from buoy_search.planning.applied_state import AppliedStateError, resolve_state_root
...
from buoy_search.indexing.github_repo import GitHubRepoError, crawl_github_repo
from buoy_search.relations.base import DatabaseRelationError
from buoy_search.indexing.repo_syntax_chunking import REPO_CHUNKING_ARMS
from buoy_search.evals.core import (
    build_dry_run_eval_report,
    load_eval_cases,
    run_live_evals,
)
from buoy_search.retrieval.evidence import (
    EvidenceCalibration,
...
def crawl_source(source: object, options: CrawlOptions) -> dict[str, object]:
    ...
    if source_kind == "duckdb_relation":
        from buoy_search.relations.duckdb import crawl_duckdb_relation

        return crawl_duckdb_relation(source, options)  # type: ignore[arg-type]
    if source_kind == "bigquery_relation":
        from buoy_search.relations.bigquery import crawl_bigquery_relation

        return crawl_bigquery_relation(source, options)  # type: ignore[arg-type]
    if source_kind == "snowflake_relation":
        from buoy_search.relations.snowflake import crawl_snowflake_relation
        ...
        with command.stage(
            ...
            if namespaces:
                ...
                if args.dry_run:
                    ...
                    evidence_assessor = (
                        CalibratedEvidenceAssessor(evidence_calibration)
                        if worker_session is None
                        else CalibratedEvidenceAssessor(
