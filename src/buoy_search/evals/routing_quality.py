...
from buoy_search.retrieval.cross_encoder import CROSS_ENCODER_MODEL, CROSS_ENCODER_REVISION
...
DEFAULT_ROUTING_CANARY_DIR = PACKAGE_DATA_DIR / "routing_canaries"
DEFAULT_ROUTING_CALIBRATION = (
    PACKAGE_DATA_DIR / "automatic_routing_confidence_calibration.json"
)
...
ROUTING_SEMANTIC_CALIBRATION_SCHEMA_VERSION = 4
...
# Active schemas v3 and v4 carry this exact set and its aggregate projection as
# the only catalog state authorized to use the certified singleton thresholds.
...
ROUTING_SEMANTIC_CONTRACT_REVISION = "bounded-prototype-routing-v1"
ROUTING_SEMANTIC_ARTIFACT_SCHEMA = "automatic-routing-confidence-v4"
ROUTING_SEMANTIC_ALGORITHM = "bounded-prototype-singleton-or-top3-v1"
ROUTING_SEMANTIC_CONFIDENT_SELECTION = (
    "singleton_when_score_and_margin_meet_floors"
)
ROUTING_SEMANTIC_FALLBACK_SELECTION = "top3_or_all_when_fewer"
ROUTING_SEMANTIC_EXPLICIT_NAMESPACE_BEHAVIOR = "bypass_automatic_routing"


def routing_semantic_compatibility_descriptor() -> dict[str, object]:
    """Return the canonical behavior-affecting routing compatibility contract."""

    return {
        "contract_revision": ROUTING_SEMANTIC_CONTRACT_REVISION,
        "artifact_schema": ROUTING_SEMANTIC_ARTIFACT_SCHEMA,
        "routing_algorithm": ROUTING_SEMANTIC_ALGORITHM,
        ...
        "confident_selection": ROUTING_SEMANTIC_CONFIDENT_SELECTION,
        "fallback_selection": ROUTING_SEMANTIC_FALLBACK_SELECTION,
        "explicit_namespace_behavior": (
            ROUTING_SEMANTIC_EXPLICIT_NAMESPACE_BEHAVIOR
        ),
...
_SEMANTIC_COMPATIBILITY_FIELDS = set(
...
@dataclass(frozen=True)
class RoutingSemanticCompatibility:
    ...
    shortlist_limit: int
    max_examples: int
    confident_selection: str
    fallback_selection: str
    explicit_namespace_behavior: str
    catalog_policy: str
...
class RoutingConfidenceCalibration:
    ...
    semantic_compatibility: RoutingSemanticCompatibility | None
...
def load_routing_confidence_calibration(
    ...
    elif schema_version == ROUTING_SEMANTIC_CALIBRATION_SCHEMA_VERSION:
...
def validate_routing_confidence_calibration(
    ...
    semantic_compatibility = calibration.semantic_compatibility
    ...
    if not _is_exact_frozen_value(
        calibration.schema_version, ROUTING_SEMANTIC_CALIBRATION_SCHEMA_VERSION
    ):
        raise ValueError("Routing confidence calibration schema is incompatible.")
    active_schema_version = ROUTING_SEMANTIC_CALIBRATION_SCHEMA_VERSION
    ...
    if type(semantic_compatibility) is not RoutingSemanticCompatibility:
        raise ValueError("Routing semantic compatibility object is invalid.")
    _require_exact_frozen_attributes(
        semantic_compatibility,
        routing_semantic_compatibility_descriptor(),
        where="routing semantic compatibility",
    )
    _require_exact_frozen_attributes(
        calibration,
        {
...
def _parse_active_routing_confidence_calibration(
    ...
    schema_version = payload.get("schema_version")
    if schema_version != ROUTING_SEMANTIC_CALIBRATION_SCHEMA_VERSION:
    ...
    bindings_payload = _parse_routing_confidence_bindings_payload(
        payload.get("bindings"), active=True, anchored=True
    )
    semantic_compatibility = _parse_routing_semantic_compatibility(
        payload.get("semantic_compatibility")
    )
...
def _parse_routing_semantic_compatibility(
    value: object,
) -> RoutingSemanticCompatibility:
    payload = _required_mapping(value, where="routing semantic compatibility")
    _require_exact_fields(
        payload,
        _SEMANTIC_COMPATIBILITY_FIELDS,
        where="routing semantic compatibility",
    )
    expected = routing_semantic_compatibility_descriptor()
    for field, expected_value in expected.items():
        if not _is_exact_frozen_value(payload.get(field), expected_value):
            raise ValueError(
                f"Routing semantic compatibility {field!r} is incompatible."
            )
    return RoutingSemanticCompatibility(
        contract_revision=cast(str, payload["contract_revision"]),
        artifact_schema=cast(str, payload["artifact_schema"]),
        routing_algorithm=cast(str, payload["routing_algorithm"]),
        ...
        confident_selection=cast(str, payload["confident_selection"]),
        fallback_selection=cast(str, payload["fallback_selection"]),
        explicit_namespace_behavior=cast(
            str, payload["explicit_namespace_behavior"]
        ),
        catalog_policy=cast(str, payload["catalog_policy"]),
    )
