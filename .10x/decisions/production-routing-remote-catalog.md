...
- Approved apply stages a remote card update and preserves manual semantics/enabled state with revision-bound conflict detection and recoverable pending state.
- Catalog CLI operations become authenticated remote operations; local path options/environment variables are removed.
