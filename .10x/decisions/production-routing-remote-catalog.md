...
- The two validated Oscilar and Dagster Benchmark cards are migrated remotely. After remote routing verification, exact `.buoy/catalog.json` is deleted only while its expected lock path remains absent; any lock appearance blocks deletion. Unregistered live Dagster/Thistle namespaces remain excluded.
