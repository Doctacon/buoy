...
Created: 2026-08-25
Updated: 2026-08-25
Superseded-By: .10x/decisions/superseded/one-time-provider-invocation-receipt-final-recovery-model-authority-correction.md
...
# One-Time Provider Invocation Receipt Final Recovery
...
5. reuse the exact integration package evidence cryptographically and MUST NOT
   author or run a novel archive member-type classifier; and
6. retain the complete accepted candidate, isolated runtime, and needed harness
...
substitution, credential-value access, model construction, provider/DNS/TLS/
network access, telemetry database/store/API/command behavior, retrieval,
global installation/tool replacement, ref mutation, or writes outside owned
temporary state. Existing caches may be read only for the permitted seed copy
and MUST remain unchanged.

Phase 1 closes only after an independent reviewer records explicit **PASS/GO**
bound to the complete sanitized preparation evidence commit/tree and the exact
retained immutable candidate/runtime. Drift, deletion, incomplete evidence,
real product defect, unresolved side effect, qualified/stale review, or NO-GO
forbids phase 2.
...
PASS/GO. It MUST NOT build, rebuild, correct a harness, substitute a candidate,
...
- cached model `BAAI/bge-small-en-v1.5` at revision
  ...
  automatic-device behavior, and offline/no-download controls; and
- the existing intended credential source inside the private child without
  exposing its value.

It then runs exactly one ordinary automatic command through the private receipt
scope. Telemetry remains disabled. Provider operations are only current strong
catalog reads and selected content reads. Receipt acceptance requires catalog
invocation count at most 5 and content invocation count at most 18. There is no
preview, explicit command, alternate case/dataset/model/candidate, substitution,
second command, provider retry, command retry, or receipt retry. Command start
consumes all live authority regardless of outcome.
...
candidate until cleanup, owner-private and source UV caches, model cache,
telemetry filesystem, credential source, global tools, processes, and unrelated
state. Provider activity MUST reconcile as read-only. Cleanup and retention MUST
follow the active lifecycle spec: on PASS retain only the canonical sanitized
...
Its former active path remains only a relative compatibility link resolving to
that `Status: superseded` record. Historical references therefore continue to
resolve but grant no current authority.
...
accepted candidate identity remain exact. Live authority cannot begin until a
complete retained candidate independently passes. The final live operation
remains singular, read-only, telemetry-off, content-free, and fail-closed.
