...
Original-Decision: .10x/decisions/superseded/one-time-live-provider-invocation-receipt-canary.md
...
The recovery preparation activation and its one permitted build were consumed
when the mandatory archive-member-type assertion returned nonzero. Preparation
stopped before installation, GO, provider/network access, or a live command.
Current owner-ratified terminal disposition is recorded at
`.10x/decisions/provider-invocation-receipt-live-canary-attempts-are-permanently-stopped.md`.
This recovery decision is historical provenance only and grants no alternate
inspection, corrected validation, preparation, investigation, successor
recovery, GO, live command, or retry authority.
