...
- Live `retrieve`, live evals, and autoresearch/eval runtime configuration MUST accept `--embedding-precision` and `BUOY_EMBEDDING_PRECISION`. Their default remains `float32`. During Buoy 0.2, `TURBO_SEARCH_EMBEDDING_PRECISION` follows the deprecated environment fallback/conflict rules in `.10x/specs/buoy-local-compatibility.md`.
