...
- `/plans/:planId`: fully verified identity, provenance, source activity, embedding contract, baseline, diff, bounded paginated changed/new chunk content, bounded paginated stale identities, warnings/errors, explicit unchanged-content omission, and read-only notices. No full-page or unchanged-content preview exists. Remote-source plans MUST state they can be reviewed without reconnecting to the source warehouse.
...
Vitest and React Testing Library cover dashboard states, namespace filtering/detail, fully verified changed/stale plan review and escaped content, removed page preview behavior, search validation/results/missing credentials, explicit remote refresh, graph placeholder, and absence of mutation controls.
