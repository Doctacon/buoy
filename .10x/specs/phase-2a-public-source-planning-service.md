...
Extract the existing CLI plan orchestration into a reusable Python application service used by both the CLI and command-center plan jobs. Phase 2A managed requests support only public HTTP(S) websites and public GitHub repository URLs. Existing CLI support for local documents and DuckDB/BigQuery/Snowflake remains unchanged but is not exposed through managed jobs.

## Request contract

A managed request contains required `source_url` and optional `max_pages_or_files`, `max_chunks`, `namespace`, `include_paths`, and `exclude_paths`. Omitted limits use current source defaults. Namespace validation and target semantics MUST match the CLI. `source_url` MUST be at most 2048 characters, use only `http` or `https`, contain no URL userinfo, and current source detection MUST classify it as a public website or supported public GitHub repository root. GitHub `/tree/` and `/blob/` URLs MUST be rejected because arbitrary refs, subtrees, and blobs are outside managed scope. Existing source detection and canonicalization govern query and fragment handling; managed validation MUST NOT reject them categorically because public website URLs may use them. No token, credential, local path, arbitrary Git ref, selector, custom chunking, or advanced source option is accepted.
