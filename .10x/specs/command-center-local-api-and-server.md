...
- The versioned `/api/v1` API MUST include local health, sanitized capabilities, dashboard, namespace list/detail, plan list/detail, paginated changed/new chunks, paginated stale rows, explicit remote snapshot, and search. Schema-v1 page list/detail routes are removed because schema-v2 plans retain no pages.
...
1. FastAPI tests cover all active endpoints, removed page-route 404 behavior, summary-qualified versus fully verified plan states, structured failures, static fallback, and unknown API behavior using temporary roots and fakes.
