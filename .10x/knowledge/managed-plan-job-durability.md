...
- Managed source work occurs in a private staging directory. After complete normal schema-v2 artifact verification, only `plan.json` and `delta.duckdb` are copied into the held final job-directory descriptor using exclusive no-follow regular-file operations, fsyncs, and source/destination hashes. No manifest, chunks JSONL, summary sidecar, pages, or source checkout is retained.
