...
from buoy_search.evals import autoresearch
...
COMMAND_CASES = (
    ("crawl", "buoy_search.cli.main._run_crawl", ["crawl", "--base-url", "https://example.com", "--json"]),
    ("plan", "buoy_search.cli.main._run_plan", ["plan", "https://example.com", "--json"]),
    ("apply interactive decline", "buoy_search.cli.main._run_apply", ["apply", "--json"]),
    ("apply dry-run", "buoy_search.cli.main._run_apply", ["apply", "--dry-run", "--json"]),
    ("apply approved", "buoy_search.cli.main._run_apply", ["apply", "--approve", "--json"]),
    (
        "retrieve",
        "buoy_search.cli.main._run_retrieve",
        ["retrieve", "question", "--namespace", "site-example-v1", "--json"],
    ),
    (
        "evals",
        "buoy_search.cli.main._run_evals",
...
class EnvironmentAliasRemovalTests(unittest.TestCase):
    ...
    def test_module_help_paths_remain_available(self) -> None:
        ...
        for module in ("buoy_search", "buoy_search.evals.autoresearch"):
    ...
    def test_autoresearch_rejects_before_experiment_or_output_access(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(
                ...
                clear=True,
            ), patch("buoy_search.evals.autoresearch.load_experiment") as load, patch(
                "buoy_search.evals.autoresearch.run_experiment"
    ...
    def test_autoresearch_help_and_parser_failure_precede_gate(self) -> None:
        ...
        self.assertEqual(raised.exception.code, 0)
        self.assertTrue(stdout.getvalue().startswith("usage: python -m buoy_search.evals.autoresearch"))
        self.assertEqual(stderr.getvalue(), "")
