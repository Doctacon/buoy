...
from buoy_search.telemetry import store as telemetry_store, writer as telemetry_writer
...
class RejectForbidden(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        if fullname in {"duckdb", "buoy_search.telemetry.store"}:
            raise AssertionError(f"forbidden eager import: {fullname}")
        return None

sys.meta_path.insert(0, RejectForbidden())
import buoy_search.telemetry.writer
assert "duckdb" not in sys.modules
assert "buoy_search.telemetry.store" not in sys.modules
"""
