...
from buoy_search.telemetry import producer as telemetry, store as telemetry_store, writer as telemetry_writer
from buoy_search.cli.main import main
from buoy_search.catalog.local import CatalogError
...
from buoy_search.catalog.remote import RemoteCatalogError
from buoy_search.retrieval.routing import AutomaticRoutingError
...
class RetrieveCommandTelemetryTests(unittest.TestCase):
        ...
        for namespaces in (("site-one-v1",), ("site-one-v1", "site-two-v1")):
            with self.subTest(namespaces=namespaces):
                ...
                target = (
                    ...
                    if len(namespaces) == 1
                    else "buoy_search.cli.main.MultiNamespaceRetriever.from_configs"
                )
            ...
            with self.subTest(name=name):
                ...
                with ExitStack() as stack:
                    ...
                    if render_failure is not None:
                        stack.enter_context(
                            patch(
                                "buoy_search.cli.main._print_json",
    ...
    def test_automatic_weak_evidence_widening_publishes_truthful_v2_graph(self) -> None:
        ...
        patches[6] = patch(
            "buoy_search.cli.main.hybrid_route", return_value=_FakeRouting(3)
        )
        with ExitStack() as stack:
            for item in patches:
                stack.enter_context(item)
    ...
    def test_configuration_and_pipeline_failures_keep_output_and_categories(self) -> None:
        ...
        stderr = StringIO()
        with patch(
            "buoy_search.cli.main.HybridRetriever.from_config", return_value=retriever
    ...
    def test_render_and_unexpected_exceptions_reraise_identity_with_exit_one(self) -> None:
        ...
        with patch(
            "buoy_search.cli.main._print_json", side_effect=escaped
        ...
        with patch(
            "buoy_search.cli.main.resolve_retrieval_namespaces", side_effect=escaped
        ...
        for enabled in (False, True):
            with self.subTest(enabled=enabled):
                ...
                with patch.dict(os.environ, environment, clear=True), patch(
                    "buoy_search.cli.main.resolve_retrieval_namespaces",
    ...
    def test_controlled_initialization_routing_and_render_delays_skip_pipeline(self) -> None:
        def explicit_duration(*, initialize_ms: int = 0, render_ms: int = 0):
            ...
            with patch.object(telemetry, "_time_ns", side_effect=lambda: now[0]), patch(
                "buoy_search.cli.main.HybridRetriever.from_config", side_effect=construct
            ), patch("buoy_search.cli.main._print_json", side_effect=render):
                self.assertEqual(
                    main(
                        [
                            "retrieve",
    ...
    def test_controlled_subprocess_probe_attributes_all_phase_delays(self) -> None:
        probe = Path(__file__).parents[1] / "fixtures" / "retrieve_command_timing_probe.py"
        injected_delay_ms = 2_000

        def observe(stage: str, delay_ms: int) -> dict[str, object]:
        ...
        for stage in ("bootstrap", "initialize", "routing", "pipeline", "render"):
            observations[stage] = {
                delay_ms: observe(stage, delay_ms)
