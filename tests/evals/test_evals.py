...
from buoy_search.evals.core import EvalCase, RepoEvalScore, aggregate_repo_scores, hit_summary, load_eval_cases, score_hits, score_repo_hits
from buoy_search.retrieval.retriever import SearchHit


BUOY_REPO_EVAL_DATASET = Path("src/buoy_search/data/buoy_search_repo_search_v3.json")
...
class EvalHarnessTests(unittest.TestCase):
    ...
    def test_load_eval_cases_accepts_graded_repo_judgments(self) -> None:
                        ...
                        "cases": [
                            {
                                ...
                                "judgments": [
                                    {
                                        "repo_path": "src/buoy_search/indexing/crawler.py",
                                        "grade": 3,
        ...
        self.assertEqual(cases[0].judgments[0].repo_path, "src/buoy_search/indexing/crawler.py")
    ...
    def test_score_repo_hits_perfect_ranking_gets_full_composite_score(self) -> None:
        ...
        score = score_repo_hits(
            case,
            [
                {"path": "src/buoy_search/indexing/crawler.py", "url": "https://github.com/owner/repo/blob/main/src/buoy_search/indexing/crawler.py"},
        ...
        summary = hit_summary(
            {
                ...
                "source_metadata": {"repo_path": "src/buoy_search/evals/core.py"},
            },
            1,
        )

        self.assertEqual(summary["path"], "generated-page.md")
        self.assertEqual(summary["repo_path"], "src/buoy_search/evals/core.py")
    ...
    def test_score_repo_hits_partial_ranking_uses_grades_and_ranks(self) -> None:
        ...
        score = score_repo_hits(
            ...
            [
                ...
                {"source_metadata": {"repo_path": "tests/test_crawler.py"}},
                {"source_metadata": {"repo_path": "tests/test_crawler.py"}},
                {"path": "src/buoy_search/indexing/crawler.py"},
            ],
        )
    ...
    def test_score_repo_hits_no_match_scores_zero(self) -> None:
        score = score_repo_hits(graded_case(), [{"path": "src/buoy_search/planning/apply.py"}])
    ...
    return EvalCase.from_mapping(
        {
            ...
            "question": "Where is GitHub repo URL routing implemented?",
            "judgments": [
                {"repo_path": "src/buoy_search/indexing/crawler.py", "grade": 3},
