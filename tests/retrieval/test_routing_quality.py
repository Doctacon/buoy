...
from buoy_search.catalog.local import ROUTING_MODEL, ROUTING_MODEL_REVISION
...
from buoy_search.evals.routing_quality import (
    ...
    ROUTING_SEMANTIC_CALIBRATION_SCHEMA_VERSION,
...
from buoy_search.evals.multi_corpus import load_multi_corpus_eval_dataset
from tests.retrieval.routing_confidence_fixtures import (
...
def active_calibration_payload() -> dict[str, object]:
    return {
        "schema_version": ROUTING_SEMANTIC_CALIBRATION_SCHEMA_VERSION,
        ...
        "semantic_compatibility": routing_semantic_compatibility_descriptor(),
        "calibration": {
...
def anchored_calibration_payload() -> dict[str, object]:
    return active_calibration_payload()
...
class RoutingConfidenceCalibrationLoaderTests(unittest.TestCase):
    ...
    def test_exact_schema_v4_active_artifact_loads_only_as_package_resource(self) -> None:
        temporary, path = self.write_payload(active_calibration_payload())
        self.addCleanup(temporary.cleanup)
    ...
    def test_schema_v4_artifact_carries_the_exact_certified_anchor(self) -> None:
    ...
    def test_schema_v4_classifies_exact_anchor_and_every_valid_drift(self) -> None:
        calibration = self.load_anchored_calibration()
        anchor_cards = [
    ...
    def test_obsolete_schema_v2_active_artifact_is_rejected(self) -> None:
        payload = active_calibration_payload()
        payload["schema_version"] = ROUTING_ACTIVE_CALIBRATION_SCHEMA_VERSION
    ...
    def test_schema_v4_rejects_mutated_anchor_authority(self) -> None:
    ...
    def test_schema_v4_active_artifact_rejects_every_frozen_authority_change(self) -> None:
        mutations = (
            ...
            ("semantic", lambda value: value["semantic_compatibility"].update({"routing_algorithm": "other-v1"})),
    ...
    def test_schema_v4_rejects_each_semantic_descriptor_mismatch(self) -> None:
        descriptor = routing_semantic_compatibility_descriptor()
        for field, original in descriptor.items():
            with self.subTest(field=field):
                payload = active_calibration_payload()
                payload["semantic_compatibility"][field] = (
                    original + 1 if type(original) is int else "incompatible-v1"
                )
                temporary, path = self.write_payload(payload)
                self.addCleanup(temporary.cleanup)
                with patch.object(
    ...
    def test_schema_v4_active_artifact_rejects_each_missing_receipt_field(
        self,
    ) -> None:
    ...
    def test_schema_v4_active_artifact_rejects_malformed_receipts(self) -> None:
        mutations = (
            ("receipt container", lambda value: value.update({"receipts": []})),
            (
                "digest",
                lambda value: value["receipts"].update(
                    {"authorization_report_sha256": "not-a-sha256"}
                ),
            ),
            (
                "commit",
    ...
    def test_schema_mode_mixtures_and_nonfinite_active_thresholds_fail(self) -> None:
        schema_four_collect = collect_calibration_payload()
        schema_four_collect["schema_version"] = ROUTING_SEMANTIC_CALIBRATION_SCHEMA_VERSION
        schema_one_active = active_calibration_payload()
        schema_one_active["schema_version"] = ROUTING_CALIBRATION_SCHEMA_VERSION
        for label, payload in (
            ("schema-four collect", schema_four_collect),
    ...
    def test_schema_version_requires_an_exact_integer_for_both_schemas(self) -> None:
        cases = (
            ...
            ("schema-v4 boolean", active_calibration_payload, True),
            ("schema-v4 float", active_calibration_payload, 4.0),
        )
        ...
        mutations = (
            ...
            (
                ...
                lambda value: replace(
                    ...
                    receipts=replace(
                        value.receipts,
                        collect_artifact_sha256=StringSubclass(
                            value.receipts.collect_artifact_sha256
                        ),
                    ),
                ),
            ),
            (
                "semantic structural stand-in",
                lambda value: replace(
                    value,
                    semantic_compatibility=SimpleNamespace(
                        **vars(value.semantic_compatibility)
                    ),
                ),
            ),
            (
