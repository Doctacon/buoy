...
from buoy_search.retrieval.routing import RoutedRetrievalResult
...
class CrossEncoderLoaderTests(unittest.TestCase):
    ...
    def test_loader_is_lazy_cached_and_pins_local_safetensors_cpu_contract(self) -> None:
        ...
        with patch.dict(sys.modules, {"sentence_transformers": module}), patch(
            "buoy_search.retrieval.cross_encoder.suppress_model_progress_bars",
...
class MultiNamespaceCliTests(unittest.TestCase):
    ...
    def test_duplicate_cli_namespace_fails_before_config(self) -> None:
        with patch("buoy_search.cli.main.config_from_args", side_effect=AssertionError("config loaded")):
            stdout = StringIO()
            stderr = StringIO()
            with redirect_stdout(stdout), redirect_stderr(stderr):
                result = main(
