...
from buoy_search.evals.multi_corpus import normalize_collected_evidence
from buoy_search.retrieval.routing import ROUTING_PROTOTYPE_STRATEGY
from buoy_search.cli.main import print_retrieval_text
...
from tests.retrieval.test_automatic_routing import (
...
class RoutingActivationCliTests(unittest.TestCase):
    ...
    def test_invalid_activation_artifact_fails_before_credentials_or_runtime(self) -> None:
        ...
        with (
            ...
            patch("buoy_search.cli.main.config_from_args", side_effect=forbidden),
            patch("buoy_search.cli.main.load_evidence_calibration", side_effect=forbidden),
        ):
            result, stdout, stderr = run_cli(
    ...
    def test_collect_artifact_preserves_legacy_router(self) -> None:
        ...
        with (
            ...
            patch("buoy_search.cli.main.read_remote_catalog", return_value=snapshot(cards)),
            patch(
                "buoy_search.cli.main.ROUTING_EMBEDDER_FACTORY",
                return_value=FixedEmbedder(),
            ),
            patch(
                "buoy_search.cli.main.prototype_route",
    ...
    def test_active_descriptor_free_route_uses_certified_prototypes(self) -> None:
        ...
        confidence = active_routing_calibration()
        with (
            patch(
                "buoy_search.cli.main.ROUTING_CONFIDENCE_FACTORY",
        ...
        assert confidence.receipts is not None
        self.assertEqual(
            authority["source_report_sha256"],
            confidence.receipts.certified_dormant_report_sha256,
        )
        self.assertEqual(
            authority["source_commit"],
            confidence.receipts.certified_dormant_source_commit,
        )
        self.assertEqual(
            authority["source_tree"],
            confidence.receipts.certified_dormant_source_tree,
        )
    ...
    def test_active_routing_embedder_load_failure_is_redacted_before_content(self) -> None:
        ...
        with (
            ...
            patch("buoy_search.cli.main.ROUTING_RERANKER_FACTORY", side_effect=forbidden),
            patch(
                "buoy_search.cli.main.MultiNamespaceRetriever.from_configs",
                side_effect=forbidden,
            ),
...
class RoutingActivationRunnerTests(unittest.TestCase):
    def test_dormant_report_binds_exact_routing_cli_and_evidence_bytes(self) -> None:
        from tests.retrieval.test_routing_quality_runner import (
            synthetic_dataset,
            synthetic_snapshot,
        )
        ...
        package_dir = Path(routing_quality_module.__file__).resolve().parent.parent
        evaluator = provenance["evaluator"]
        for field, source_path in (
            ("routing_module_sha256", package_dir / "retrieval" / "routing.py"),
            ("cli_module_sha256", package_dir / "cli" / "main.py"),
            ("evidence_module_sha256", package_dir / "retrieval" / "evidence.py"),
        ):
            self.assertEqual(
                evaluator[field],
    ...
    def test_activation_gate_recomputes_calibration_and_semantic_contract(self) -> None:
        from tests.retrieval.test_routing_quality_runner import synthetic_dataset
        ...
        self.assertIsNotNone(confidence.semantic_compatibility)
        calibration = confidence.calibration
        semantic = confidence.semantic_compatibility
        ...
        mismatches = (
            ...
            (
                ...
                replace(
                    ...
                    semantic_compatibility=replace(
                        semantic, routing_algorithm="other-v1"
                    ),
                ),
                True,
            ...
            (
                "routing model revision",
                replace(
                    confidence,
                    semantic_compatibility=replace(
                        semantic, routing_model_revision="other-revision"
                    ),
                ),
                True,
                "confidence_semantic_compatibility",
    ...
    def test_evaluation_contract_accepts_both_prototype_route_reasons(self) -> None:
        from tests.evals.test_multi_corpus_eval_runner import active_evidence
