...
from buoy_search import _provider_invocation_receipt as receipt
from buoy_search.catalog import CardFields, NamespaceCard, prepare_card
...
class CatalogInvocationInstrumentationTests(unittest.TestCase):
    ...
    def test_multipage_aggregates_preserve_passes_arguments_and_order(self) -> None:
        ...
        with patch("buoy_search.remote_catalog.CARD_PAGE_SIZE", 2):
            snapshot = read_remote_catalog(
                client,
                region=REGION,
                compatibility=compatibility(),
    ...
    def test_post_return_local_error_and_interruption_do_not_fabricate_call_failure(self) -> None:
        ...
        observer = RecordingObserver()
        caught = None
        with patch("buoy_search.remote_catalog.validate_remote_schema", side_effect=interruption):
            try:
                read_remote_catalog(
                    client,
                    region=REGION,
    ...
    def test_page_bound_failures_happen_after_only_reached_expressions(self) -> None:
        ...
        observer = RecordingObserver()
        with patch("buoy_search.remote_catalog.CARD_PAGE_SIZE", 1), patch(
            "buoy_search.remote_catalog.MAX_PAGES_PER_PASS", 1
        ):
    ...
    def test_observer_faults_preserve_expression_result_exception_and_full_read(self) -> None:
        from buoy_search.remote_catalog import _invoke_catalog_expression
