...
from buoy_search.indexing.crawler import (
...
class CrawlerHelperTests(unittest.TestCase):
    def test_diagnostic_clock_failures_are_best_effort(self) -> None:
        with patch("buoy_search.indexing.crawler.time.monotonic", side_effect=RuntimeError("clock failed")):
    ...
    def test_incremental_resource_read_accepts_exact_robots_and_sitemap_boundaries(self) -> None:
        ...
        for url, ceiling, limit_type in cases:
            with self.subTest(limit_type=limit_type):
                ...
                with patch(
                    "buoy_search.indexing.crawler._NO_REDIRECT_OPENER.open",
    ...
    def test_redirected_declared_gzip_fails_closed_before_link_fallback(self) -> None:
        ...
        with patch(
            "buoy_search.indexing.crawler._NO_REDIRECT_OPENER.open", side_effect=fake_open
        ):
            with patch("buoy_search.indexing.crawler.build_link_spider_class") as link_mock:
                with self.assertRaisesRegex(
                    SitemapResourceError,
    ...
    def test_multiple_sitemap_queue_propagates_late_limit_error(self) -> None:
        ...
        with patch(
            "buoy_search.indexing.crawler.fetch_url_bytes", side_effect=fake_fetch
        ):
    ...
    def test_sitemap_resource_errors_never_start_link_fallback(self) -> None:
        ...
        for error in errors:
            with self.subTest(error=str(error)):
                with patch(
                    "buoy_search.indexing.crawler.discover_sitemap_page_urls",
                    side_effect=error,
                ):
                    with patch(
                        "buoy_search.indexing.crawler.build_link_spider_class"
                    ) as link_mock:
    ...
    def test_language_policy_all_keeps_localized_paths(self) -> None:
        ...
        with patch("buoy_search.indexing.crawler.discover_sitemap_page_urls", return_value=urls) as discover_mock:
            effective, report = apply_language_policy(options)
    ...
    def test_crawl_pdf_rejects_empty_markitdown_extraction(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("buoy_search.indexing.crawler.markitdown_pdf_to_markdown", return_value=" \n"):
                with self.assertRaisesRegex(RuntimeError, "No text was extracted"):
    ...
    def test_crawl_local_file_rejects_empty_markitdown_extraction(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("buoy_search.indexing.crawler.markitdown_file_to_markdown", return_value=" \n"):
                with self.assertRaisesRegex(RuntimeError, "No text was extracted"):
                    crawl_local_document(source, options)
    ...
    def test_crawled_page_from_response_skips_unextractable_html(self) -> None:
        ...
        with patch("buoy_search.indexing.crawler.markdown_from_response", side_effect=ValueError("bad html")):
    ...
    def test_hybrid_crawl_merges_sitemap_and_link_pages(self) -> None:
        ...
        with patch("buoy_search.indexing.crawler.build_sitemap_spider_class", return_value=SitemapSpider):
            with patch("buoy_search.indexing.crawler.build_link_spider_class", return_value=LinkSpider):
                with patch("buoy_search.indexing.crawler.run_scrapling_spider", side_effect=fake_run):
    ...
    def test_default_sitemap_strategy_skips_link_crawl_when_sitemap_has_pages(self) -> None:
        ...
        with patch("buoy_search.indexing.crawler.build_sitemap_spider_class", return_value=SitemapSpider):
            with patch("buoy_search.indexing.crawler.build_link_spider_class", return_value=LinkSpider) as link_mock:
                with patch("buoy_search.indexing.crawler.run_scrapling_spider", side_effect=fake_run):
                    pages, stats, strategy = crawl_pages(
                        options,
                        sitemap_page_urls=["https://example.com/sitemap-page"],
    ...
    def test_link_crawl_strategy_skips_sitemap(self) -> None:
        ...
        with patch("buoy_search.indexing.crawler.build_sitemap_spider_class", return_value=SitemapSpider) as sitemap_mock:
            with patch("buoy_search.indexing.crawler.build_link_spider_class", return_value=LinkSpider):
                with patch("buoy_search.indexing.crawler.run_scrapling_spider", return_value=([link_page], {"requests_count": 1})):
