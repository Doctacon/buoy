...
from buoy_search.planning.applied_state import AppliedStateRow, build_applied_state, load_applied_state, save_applied_state
from buoy_search.planning.apply import apply_preflight_summary, load_verified_apply_plan
from buoy_search.indexing.crawler import CrawlOptions, parse_github_repo_url
from buoy_search.indexing.chunker import parse_markdown_file, process_corpus
from buoy_search.planning.plan_artifacts import build_generic_site_row, build_plan_artifacts, verify_plan_artifacts, write_plan_artifacts
from buoy_search.planning.plan_diff import diff_manifest_against_state
from buoy_search.indexing.repo_syntax_chunking import RepoSyntaxChunkingError, chunk_source, source_payload
from buoy_search.indexing.treatment_token_budget import (
    TreatmentTokenBudgetError,
    exact_token_count,
    load_pinned_tokenizer,
)
from buoy_search.indexing.github_repo import (
    GitHubRepoError,
    GitHubRepoMetadata,
    acquire_github_repo,
...
class GitHubRepoAcquisitionTests(unittest.TestCase):
    ...
    def test_acquire_github_repo_shallow_clones_and_records_commit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("buoy_search.indexing.github_repo.resolve_github_repo_metadata", return_value=metadata):
    ...
    def test_acquire_github_repo_rejects_link_as_tree_subdirectory(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "buoy_search.indexing.github_repo.resolve_github_repo_metadata",
    ...
    def test_portable_reader_retains_regular_file_and_rejects_link(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("buoy_search.indexing.github_repo.os.supports_dir_fd", set()):
            ...
            for unsafe in (
                ...
                with (
                    ...
                    patch("buoy_search.indexing.github_repo.os.supports_dir_fd", set()),
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for marked_path in (checkout, checkout / "nested", candidate):
                ...
                with (
                    ...
                    patch("buoy_search.indexing.github_repo.os.supports_dir_fd", set()),
                    patch("buoy_search.indexing.github_repo.os.lstat", side_effect=reparse_lstat),
                    patch("buoy_search.indexing.github_repo.os.read") as read_mock,
            ...
            real_observe = __import__(
                "buoy_search.indexing.github_repo", fromlist=["_portable_path_observations"]
            ...
            with (
                patch("buoy_search.indexing.github_repo.os.supports_dir_fd", set()),
                patch(
                    "buoy_search.indexing.github_repo._portable_path_observations",
    ...
    def test_oversize_reader_reads_only_the_requested_sample(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("buoy_search.indexing.github_repo.os.read", side_effect=recording_read):
                snapshot = read_repo_regular_file(
                    checkout,
                    "large.txt",
                    max_file_bytes=100,
    ...
    def test_explicit_current_default_matches_no_arm_pages_chunks_ids_and_citations(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("buoy_search.indexing.github_repo.acquire_github_repo", return_value=acquisition):
    ...
    def test_header_and_prose_overages_are_independent_fail_closed_gates(self) -> None:
        ...
        for files, marker, expected_class in cases:
            with self.subTest(row_class=expected_class), tempfile.TemporaryDirectory() as tmp:
                ...
                corpus = build_github_repo_corpus(acquisition, root / "pages")
                with patch(
                    "buoy_search.indexing.github_repo.load_pinned_tokenizer",
