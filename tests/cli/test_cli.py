...
from buoy_search.planning.applied_state import AppliedStateRow, applied_state_paths, build_applied_state, save_applied_state
from buoy_search.cli.main import OneLineProgress, build_parser, main, print_eval_text, print_retrieval_text
from buoy_search.indexing.crawler import CrawlExecution, CrawlOptions, parse_github_repo_url
from buoy_search.indexing.chunker import process_corpus
from buoy_search.indexing.github_repo import GitHubRepoAcquisition, GitHubRepoMetadata, GitTreeEntry
...
class CliTests(unittest.TestCase):
    ...
    def test_retrieve_rejects_json_and_explain_before_runtime_work(self) -> None:
        ...
        with patch.dict(
            ...
            "buoy_search.cli.main.removed_embedding_environment_error",
            side_effect=AssertionError("environment compatibility inspected"),
        ), patch(
            "buoy_search.cli.main.load_config",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                with patch("buoy_search.local_paths.Path.home", return_value=home), patch(
                    "buoy_search.planning.planning_service.crawl_source_with_plan", side_effect=fake_crawl
                ), redirect_stdout(stdout), redirect_stderr(stderr):
    ...
    def test_crawl_default_home_resolution_failure_is_clean_and_precedes_crawl(self) -> None:
        ...
        ), patch("buoy_search.cli.main.crawl_site") as crawl_mock, redirect_stdout(stdout), redirect_stderr(stderr):
    ...
    def test_crawl_command_is_dry_run_and_needs_no_credentials(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stdout = StringIO()
            with patch("buoy_search.cli.main.crawl_site", return_value=fake_summary) as crawl_mock:
                with redirect_stdout(stdout):
                    result = main(
                        [
                            "crawl",
    ...
    def test_crawl_text_output_warns_when_caps_are_hit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stdout = StringIO()
            with patch("buoy_search.local_paths.Path.home", return_value=home), patch(
                "buoy_search.cli.main.crawl_site", return_value=fake_summary
            ):
                with redirect_stdout(stdout):
    ...
    def test_crawl_command_defaults_to_sitemap_strategy(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            home.mkdir()
            with patch("buoy_search.local_paths.Path.home", return_value=home), patch(
                "buoy_search.cli.main.crawl_site", side_effect=fake_crawl
            ):
                with redirect_stdout(stdout):
                    result = main(
                        [
    ...
    def test_crawl_command_routes_github_repo_urls_to_repo_crawler(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stdout = StringIO()
            with patch("buoy_search.cli.main.crawl_github_repo", side_effect=fake_github_crawl) as github_mock:
                with patch("buoy_search.cli.main.crawl_site") as site_mock:
                    with redirect_stdout(stdout):
                        result = main(
                            [
                                "crawl",
    ...
    def test_crawl_command_propagates_opt_in_repo_chunking_arm(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stdout = StringIO()
            with patch("buoy_search.cli.main.crawl_github_repo", side_effect=fake_github_crawl):
                with redirect_stdout(stdout):
                    result = main(
                        [
                            "crawl",
    ...
    def test_repo_chunking_arm_rejects_non_repository_source_before_crawl(self) -> None:
        stderr = StringIO()
        with patch("buoy_search.cli.main.crawl_site") as crawl_mock:
    ...
    def test_crawl_command_accepts_local_pdf_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stdout = StringIO()
            with patch(
                "buoy_search.indexing.crawler.markitdown_pdf_to_markdown",
    ...
    def test_crawl_command_accepts_supported_local_file_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stdout = StringIO()
            with patch(
                "buoy_search.indexing.crawler.markitdown_file_to_markdown",
    ...
    def test_plan_command_writes_artifacts_and_first_apply_diff_without_credentials(self) -> None:
        ...
        with patch(__name__ + ".process_corpus", wraps=process_corpus) as process_mock, patch(
            "buoy_search.indexing.crawler.crawl_site_with_plan", side_effect=fake_crawl
        ) as crawl_mock:
            with redirect_stdout(stdout):
                result = main(
                    [
    ...
    def test_plan_command_text_output_succeeds_without_state_path(self) -> None:
        ...
        stdout = StringIO()
        with patch("buoy_search.indexing.crawler.crawl_site_with_plan", side_effect=fake_crawl), redirect_stdout(stdout):
            result = main(
                [
                    "plan",
                    "https://example.com/docs/",
    ...
    def test_plan_command_removes_verified_superseded_same_namespace_plan(self) -> None:
        ...
        stdout = StringIO()
        with patch("buoy_search.indexing.crawler.crawl_site_with_plan", side_effect=fake_crawl):
            with redirect_stdout(stdout):
                result = main(
                    [
                        "plan",
    ...
    def test_plan_command_stops_default_docs_version_warning_before_crawl(self) -> None:
        ...
        stdout = StringIO()
        stderr = StringIO()
        with patch("buoy_search.indexing.crawler.discover_sitemap_page_urls", return_value=urls):
            with patch("buoy_search.indexing.crawler.crawl_pages") as crawl_pages_mock:
                with redirect_stdout(stdout), redirect_stderr(stderr):
                    result = main(
                        [
                            "plan",
    ...
    def test_plan_command_routes_github_repo_urls_to_repo_corpus_and_artifacts(self) -> None:
        ...
        stdout = StringIO()
        with patch(__name__ + ".process_corpus", wraps=process_corpus) as process_mock, patch(
            "buoy_search.indexing.github_repo.crawl_github_repo_with_plan", side_effect=fake_github_crawl
        ) as github_mock:
            with patch("buoy_search.indexing.crawler.crawl_site_with_plan") as site_mock:
                with redirect_stdout(stdout):
                    result = main(
                        [
                            "plan",
    ...
    def test_plan_command_propagates_repo_chunking_arm_into_bounded_artifacts(self) -> None:
        ...
        stdout = StringIO()
        with patch("buoy_search.indexing.github_repo.acquire_github_repo", return_value=acquisition), patch(
            "buoy_search.indexing.github_repo.list_tracked_files",
            return_value=[
                GitTreeEntry(
                    mode="100644",
                    object_type="blob",
    ...
    def test_plan_command_writes_pdf_artifacts_without_source_path(self) -> None:
        ...
        ), patch("buoy_search.indexing.crawler.process_corpus", wraps=process_corpus) as process_mock:
            with patch("buoy_search.indexing.crawler.crawl_site_with_plan") as site_mock:
                with patch("buoy_search.indexing.github_repo.crawl_github_repo_with_plan") as github_mock:
                    with redirect_stdout(stdout):
                        result = main(
                            [
                                "plan",
