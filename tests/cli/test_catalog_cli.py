...
from buoy_search.catalog.local import CardFields, CatalogError, ROUTING_DIMENSIONS, prepare_card
...
class CatalogCliTests(unittest.TestCase):
    ...
    def test_repair_apply_defaults_to_global_state_before_remote_work(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("buoy_search.local_paths.Path.home", return_value=home), patch(
                "buoy_search.planning.apply.load_verified_catalog_repair_plan",
    ...
    def test_list_filters_orders_classifies_and_redacts_vectors(self) -> None:
        ...
        read = patch("buoy_search.cli.catalog.read_remote_catalog", return_value=snapshot)
        ...
        result, stdout, stderr = run_cli(["catalog", "list", "--all", "--json"], patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=snapshot),
        ))
        self.assertEqual((result, stderr), (0, ""))
    ...
    def test_show_live_stale_vector_visibility_and_missing_failure(self) -> None:
        ...
        result, stdout, stderr = run_cli(["catalog", "show", card.namespace, "--json"], patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=live),
        ))
        self.assertEqual((result, stderr), (0, ""))
        payload = json.loads(stdout)
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(patch("buoy_search.cli.catalog.read_remote_catalog", return_value=stale),),
        )
        self.assertEqual((result, stderr), (0, ""))
        ...
        result, stdout, stderr = run_cli(["catalog", "show", "missing", "--json"], patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=make_snapshot()),
        ))
        self.assertEqual((result, stdout), (2, ""))
    ...
    def test_credentials_are_required_before_factory_use(self) -> None:
        ...
        result, stdout, stderr = run_cli(
            ["catalog", "list", "--json"],
            env={},
            patches=(patch("buoy_search.cli.catalog.REMOTE_CATALOG_CLIENT_FACTORY", factory),),
        )
        self.assertEqual((result, stdout), (2, ""))
    ...
    def test_upsert_create_and_update_use_remote_conditions_and_redact_vector(self) -> None:
        ...
        result, stdout, stderr = run_cli(upsert_args(), patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=empty),
            patch("buoy_search.cli.catalog.create_remote_cards", create_preview),
            patch("buoy_search.catalog.local.load_routing_embedder", return_value=embedder),
        ))
        self.assertEqual((result, stderr), (0, ""))
        ...
        result, stdout, stderr = run_cli([*upsert_args(), "--approve"], patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", side_effect=[empty, final]),
            patch("buoy_search.cli.catalog.create_remote_cards", create),
            patch("buoy_search.catalog.local.load_routing_embedder", return_value=embedder),
        ))
        self.assertEqual((result, stderr), (0, ""))
        payload = json.loads(stdout)
        ...
        result, stdout, stderr = run_cli(args, patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=make_snapshot(final_card)),
            patch("buoy_search.cli.catalog.update_remote_card", update),
            patch("buoy_search.catalog.local.load_routing_embedder", return_value=FixedEmbedder()),
        ))
        self.assertEqual((result, stdout), (2, ""))
    ...
    def test_routing_example_preview_and_reader_first_v1_write_gate(self) -> None:
        ...
        result, stdout, stderr = run_cli(args, patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=live_v1),
            patch("buoy_search.cli.catalog.create_remote_cards", write),
            patch("buoy_search.catalog.local.load_routing_embedder", return_value=embedder),
        ))
        ...
        result, stdout, stderr = run_cli([*args, "--approve"], patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=live_v1),
            patch("buoy_search.cli.catalog.create_remote_cards", write),
            patch("buoy_search.catalog.local.load_routing_embedder", model),
        ))
        self.assertEqual((result, stdout), (2, ""))
    ...
    def test_routing_example_approval_uses_observed_v2_schema(self) -> None:
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                patch(
                    "buoy_search.cli.catalog.read_remote_catalog",
                    side_effect=[empty, final],
                ),
                patch("buoy_search.cli.catalog.create_remote_cards", create),
                patch(
                    "buoy_search.catalog.local.load_routing_embedder",
    ...
    def test_upsert_preserves_apply_owned_routing_passage_bank(self) -> None:
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                ...
                patch(
                    "buoy_search.cli.catalog.update_remote_card",
                    side_effect=update,
                ),
                patch(
                    "buoy_search.catalog.local.load_routing_embedder",
    ...
    def test_upsert_does_not_evict_passages_for_manual_examples(self) -> None:
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                ...
                patch("buoy_search.cli.catalog.update_remote_card", no_write),
                patch("buoy_search.catalog.local.load_routing_embedder", no_model),
            ),
        )
    ...
    def test_schema_v2_migration_preview_approval_and_idempotence(self) -> None:
        ...
        result, stdout, stderr = run_cli(
            ["catalog", "migrate-routing-v2", "--json"],
            patches=(
                patch("buoy_search.cli.catalog.read_remote_catalog", return_value=v1),
                patch(
                    "buoy_search.cli.catalog.migrate_remote_catalog_schema_v2",
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                ...
                patch("buoy_search.cli.catalog.load_config", config_read),
            ),
        )
        self.assertEqual((result, stderr), (0, ""))
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                patch("buoy_search.cli.catalog.read_remote_catalog", return_value=v2),
                patch(
                    "buoy_search.cli.catalog.migrate_remote_catalog_schema_v2",
    ...
    def test_schema_v3_migration_preview_approval_and_idempotence(self) -> None:
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                patch("buoy_search.cli.catalog.read_remote_catalog", return_value=v2),
                patch(
                    "buoy_search.cli.catalog.migrate_remote_catalog_schema_v3",
                    side_effect=AssertionError("preview wrote"),
                ),
            ),
        )
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                patch("buoy_search.cli.catalog.read_remote_catalog", return_value=v3),
                patch(
                    "buoy_search.cli.catalog.migrate_remote_catalog_schema_v3",
    ...
    def test_schema_migration_bindings_preflight_and_failed_attempt_are_bounded(self) -> None:
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                patch("buoy_search.cli.catalog.read_remote_catalog", return_value=v1),
                patch("buoy_search.cli.catalog.migrate_remote_catalog_schema_v2", no_write),
            ),
        )
        self.assertEqual((result, stderr), (2, ""))
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                patch("buoy_search.cli.catalog.read_remote_catalog", return_value=v1),
                patch("buoy_search.cli.catalog.remote_catalog_resource", no_resource),
                patch("buoy_search.cli.catalog.migrate_remote_catalog_schema_v2", no_write),
            ),
        )
        self.assertEqual((result, stderr), (2, ""))
    ...
    def test_dedicated_routing_example_preview_approval_and_idempotence(self) -> None:
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                ...
                patch("buoy_search.cli.catalog.update_remote_card", side_effect=write),
                patch(
                    "buoy_search.catalog.local.load_routing_embedder",
                    return_value=FixedEmbedder(),
                ),
                patch(
                    "buoy_search.cli.catalog.utc_now",
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                ...
                patch(
                    "buoy_search.cli.catalog.remote_catalog_resource",
        ...
        for source_uri in (" https://example.com", "https://example.com:not-a-port", "urn:example"):
            with self.subTest(source_uri=source_uri):
                result, stdout, stderr = run_cli(upsert_args(source_uri=source_uri), patches=(
                    patch("buoy_search.cli.catalog.read_remote_catalog", return_value=live),
                    patch("buoy_search.cli.catalog.create_remote_cards", create),
                    patch("buoy_search.catalog.local.load_routing_embedder", side_effect=AssertionError("model loaded")),
                ))
                self.assertEqual((result, stdout), (2, ""))
    ...
    def test_enable_disable_idempotence_and_conditional_conflict(self) -> None:
        ...
        result, stdout, stderr = run_cli(["catalog", "disable", enabled.namespace, "--json"], patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=make_snapshot(enabled)),
            patch("buoy_search.cli.catalog.update_remote_card", preview_write),
        ))
        self.assertEqual((result, stderr), (0, ""))
        preview = json.loads(stdout)
        self.assertFalse(preview["approved"])
        ...
        result, stdout, stderr = run_cli(
            ...
            patches=(
                patch("buoy_search.cli.catalog.read_remote_catalog", side_effect=[make_snapshot(enabled), make_snapshot(disabled)]),
                patch("buoy_search.cli.catalog.update_remote_card", update),
            ),
        )
        self.assertEqual((result, stderr), (0, ""))
        ...
        result, stdout, stderr = run_cli(["catalog", "disable", disabled.namespace, "--approve", "--json"], patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", side_effect=[make_snapshot(disabled), make_snapshot(disabled)]),
            patch("buoy_search.cli.catalog.update_remote_card", side_effect=AssertionError("idempotent toggle wrote")),
        ))
        self.assertEqual((result, stderr), (0, ""))
        unchanged = json.loads(stdout)
        self.assertEqual(unchanged["mutation_status"], "unchanged")
        ...
        result, stdout, stderr = run_cli(["catalog", "enable", disabled.namespace, "--approve", "--json"], patches=(
            patch("buoy_search.cli.catalog.read_remote_catalog", return_value=make_snapshot(disabled)),
            patch("buoy_search.cli.catalog.update_remote_card", side_effect=RemoteCatalogError(
