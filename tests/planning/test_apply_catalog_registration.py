...
from buoy_search.planning.applied_state import load_applied_state, save_applied_state
...
from tests.cli.test_apply_cli import (
    FakeEmbedder,
    FakeWriter,
    build_saved_plan,
    reset_fakes,
)
from tests.cli.test_catalog_cli import run_cli
            ...
            for existing in (None, manual_card(namespace)):
                with self.subTest(
                    ...
                    result, _stdout, stderr = run_cli(
                        ...
                        patches=(
                            patch(
                                "buoy_search.planning.apply.REMOTE_CATALOG_CLIENT_FACTORY",
                                return_value=FakeClient(),
                            ),
                            patch(
                                "buoy_search.planning.apply.read_remote_catalog",
                                side_effect=read_catalog,
                            ),
                            patch(
                                "buoy_search.planning.apply.create_remote_cards",
                                side_effect=write,
                            ),
                            patch(
                                "buoy_search.planning.apply.update_remote_card",
                                side_effect=write,
                            ),
                            patch(
                                "buoy_search.catalog.local.load_routing_embedder",
                            ...
                            patch(
                                "buoy_search.planning.plan_cleanup.cleanup_applied_plan_directory",
                                return_value=[],
                            ),
                        ),
                    )
