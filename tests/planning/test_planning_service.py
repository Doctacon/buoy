...
from buoy_search.indexing.crawler import CrawlExecution, CrawlOptions
from buoy_search.relations.base import DatabaseRelationError
from buoy_search.planning.plan_artifacts import verify_plan_artifacts, write_plan_artifacts
from buoy_search.planning.planning_service import (
...
class PlanningServiceTests(unittest.TestCase):
    ...
    def test_cli_database_source_is_constructed_once_and_uses_service_dispatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stderr = StringIO()
            with patch(
                "buoy_search.relations.duckdb.duckdb_relation_source",
                return_value=source,
            ) as constructor, patch(
                "buoy_search.planning.planning_service.crawl_source_with_plan",
                side_effect=DatabaseRelationError("service default dispatch reached"),
            ) as service_dispatch, patch(
                "buoy_search.cli.main.source_from_cli_args",
    ...
    def test_cli_delegates_typed_options_and_preserves_json_output(self) -> None:
        ...
        with patch(
            "buoy_search.cli.main.PlanningService.plan", return_value=service_result
