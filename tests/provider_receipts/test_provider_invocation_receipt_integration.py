...
from buoy_search.cli import main as cli
from buoy_search.telemetry.producer import CommandTelemetry
from tests.provider_receipts.test_provider_invocation_receipt_catalog import (
    REGION,
    FakeClient,
    make_card,
    make_client,
)
from tests.provider_receipts.test_provider_invocation_receipt_content import (
    PRIVATE_CONTENT,
    PRIVATE_ERROR,
    PRIVATE_NAMESPACE,
    PRIVATE_PATH,
...
def automatic_runtime(
    ...
    *,
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch("buoy_search.cli.main.ROUTING_CONFIDENCE_FACTORY", return_value=SimpleNamespace(mode="collect"))
        )
        stack.enter_context(
            patch("buoy_search.cli.main.load_evidence_calibration", return_value=EVIDENCE_CALIBRATION)
        )
        stack.enter_context(
            patch("buoy_search.cli.main.REMOTE_CATALOG_CLIENT_FACTORY", return_value=client)
        )
        stack.enter_context(
            patch("buoy_search.cli.main.ROUTING_EMBEDDER_FACTORY", return_value=object())
        )
        stack.enter_context(patch("buoy_search.cli.main.hybrid_route", side_effect=route))
        stack.enter_context(
            patch("buoy_search.cli.main.CalibratedEvidenceAssessor", return_value=CollectAssessor())
        )
...
class AutomaticReceiptIntegrationTests(unittest.TestCase):
    ...
    def test_pre_call_partial_and_all_failure_receipts_are_terminal(self) -> None:
        ...
        with receipt._provider_invocation_receipt_scope() as pre_handle, patch(
            "buoy_search.retrieval.retriever.build_multi_query_subqueries", side_effect=exact
        ):
            try:
                run_automatic(
                    pre_client,
    ...
    def test_independent_concurrent_automatic_preview_scopes_do_not_cross_account(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch("buoy_search.cli.main.REMOTE_CATALOG_CLIENT_FACTORY", side_effect=client_factory)
            )
            stack.enter_context(
                patch("buoy_search.cli.main.ROUTING_EMBEDDER_FACTORY", return_value=object())
            )
            stack.enter_context(patch("buoy_search.cli.main.hybrid_route", side_effect=route))
            stack.enter_context(patch("buoy_search.cli.main._render_retrieve"))
...
class ReceiptIntegrationAuditTests(unittest.TestCase):
    def test_only_automatic_cli_production_caller_passes_private_observer(self) -> None:
        ...
        expected = {
            ...
            "src/buoy_search/planning/apply.py": 2,
            "src/buoy_search/cli/catalog.py": 9,
            "src/buoy_search/cli/main.py": 1,
        }
        ...
        self.assertEqual(observer_callers, ["src/buoy_search/cli/main.py"])
    ...
    def test_no_public_cli_environment_persistence_or_telemetry_v2_drift(self) -> None:
        ...
        core_source = (
            root / "src/buoy_search/retrieval/_provider_invocation_receipt.py"
