...
from buoy_search.relations.bigquery import BigQueryRelationError, BigQueryRelationSource
from buoy_search.cli.main import build_parser, main, source_from_cli_args
from buoy_search.relations.base import DatabaseDocument, DatabaseScanResult
from buoy_search.relations.duckdb import DuckDBRelationSource
from buoy_search.relations.snowflake import SnowflakeRelationError, SnowflakeRelationSource
from buoy_search.planning.plan_artifacts import verify_plan_artifacts
...
class DatabaseRelationCliTests(unittest.TestCase):
    ...
    def test_importing_cli_for_apply_does_not_import_database_adapters(self) -> None:
        process = subprocess.run(
            [
                ...
                "assert not ({'buoy_search.relations.duckdb', 'buoy_search.relations.bigquery', "
                "'buoy_search.relations.snowflake'} & set(sys.modules))",
            ],
            check=False,
            capture_output=True,
            text=True,
    ...
    def test_bigquery_plan_writes_source_independent_artifacts_and_truthful_summary(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out = root / "plan"
            with patch(
                "buoy_search.relations.bigquery.scan_bigquery_relation", return_value=scan
            ):
            ...
            self.assertEqual(summary["namespace"], "customer-conversations")
            self.assertNotIn("catalog_registration", summary)
            ...
            with (
                patch("buoy_search.relations.bigquery._load_bigquery", side_effect=AssertionError("source access")),
                patch("buoy_search.relations.snowflake._load_connector", side_effect=AssertionError("source access")),
            ):
    ...
    def test_remote_plan_and_crawl_reach_lazy_dependency_boundary(self) -> None:
        commands = [
            (
                ...
                "buoy_search.relations.bigquery._load_bigquery",
            ...
            (
                ...
                "buoy_search.relations.snowflake._load_connector",
