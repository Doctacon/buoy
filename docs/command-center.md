...
Dashboard, namespace, and plan summaries share a locked, process-local snapshot cache with a 1.0-second lifetime. Successful in-process managed-plan publication invalidates that snapshot immediately; CLI or other-process changes may remain invisible until the next refresh after the lifetime expires. The cache is bounded to summary records and isolated errors, is neither persisted nor shared between processes, and never authorizes apply or artifact access. Direct plan and namespace misses force one refresh. Selected plan detail, changed-chunk pages, and stale-row pages still perform complete identity, schema, logical, source, baseline, and payload verification on every request before materializing the requested bounded window. That verification remains linear in the selected delta size, so these detail routes can be materially slower than cached summary navigation; no universal constant-time or subsecond claim is made.
